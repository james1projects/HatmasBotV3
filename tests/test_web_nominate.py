...
class FakeGodPool:
    ...
    async def do_nominate(self, user_uuid, raw_god, is_broadcaster=False,
                          use_aspect=False, display_name=None):
        self.calls.append((user_uuid, raw_god, is_broadcaster, use_aspect))
        return self.result
...
async def test_no_session():
    ...
    try:
        ...
        assert data["error"] == "Log in first."
    finally:
