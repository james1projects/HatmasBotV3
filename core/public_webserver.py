...
class PublicWebServer:
    ...
    def _setup_routes(self):
        ...
        # Aspect-capable god names (wiki-refreshed via
        # core/aspect_roster). The community page uses this to gate
        # the "Aspect" checkboxes per selected god.
        self.app.router.add_get("/api/aspects", self._handle_api_aspects)
        ...
        # aspect-icon.png is the small orange in-game Aspect marker,
        # shown next to aspect requests in the queue + pool renderers.
    ...
    async def _handle_api_community(self, request: web.Request) -> web.Response:
                ...
                if isinstance(raw, list):
                    for idx, item in enumerate(raw[:10]):
                        ...
                        god_queue.append({
                            ...
                            # Aspect requests get the aspect icon badge.
        ...
        try:
            async with self._db.execute("""
                SELECT god_name, use_aspect, added_by, vote_count,
                       added_at
                  FROM god_pool
    ...
    async def _handle_api_aspects(self, request: web.Request) -> web.Response:
        """GET /api/aspects — display names of gods that have an
        Aspect. Refreshed from the wiki by the bot's roster loop;
        this handler only reads the in-memory set. Short cache so a
        mid-session refresh (new aspect shipped) propagates within
        the hour."""
        try:
            names = aspect_roster.aspect_names()
        except Exception as e:
            print(f"[PublicWebServer] aspect list read failed: {e}")
            names = []
        return web.json_response(
            {"gods": names, "total": len(names)},
    ...
    async def _handle_api_nominate(self, request: web.Request):
        ...
        # Aspect checkbox on the community page. do_nominate also
        # detects the word "aspect" inside the god string itself, so
        # API callers can use either form.
        use_aspect = bool(body.get("aspect"))
        ...
        if not result.get("ok"):
            ...
            if reason == "no_aspect":
                return err(400, f"{result.get('god')} doesn't have an "
                                f"Aspect in SMITE 2 (yet).")
            if reason == "already_voted":
                return err(409, f"You already nominated "
                                f"{display_god(result.get('god'), result.get('use_aspect'))} "
    ...
    async def _handle_priority_create(
        ...
        try:
            ...
            # bad_twitch_username | unknown_god:<raw> | no_aspect:<god>
            return web.json_response(
