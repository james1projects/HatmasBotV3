...
streamdeck/                 ALL Stream Deck-facing launchers in one folder — see docs/STREAM_DECK.md for the button layout and wiring guide. Every relative-path .bat here starts with pushd "%~dp0.." so it operates from the repo root. Button icons live in assets/streamdeck_icons/ (regenerate via tools/make_streamdeck_icons.py). run_bot.bat stays at the repo root (Task Scheduler + bot_ctl.ps1 reference it there).
  check_stream.bat          Pre-stream readiness wrapper around tools/check_stream_ready.py. Colored report with hints, pauses 8s on success / 30s on failure. Press ~30s before going live.
