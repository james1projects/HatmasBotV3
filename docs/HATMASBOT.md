...
tools/
  ...
  bot_ctl.ps1               Start/restart/stop HatmasBot from a button. Finds supervisor + main.py processes via CIM; restart kills the bot child so the supervisor relaunches it (~5s), or opens run_bot.bat if nothing is running; stop kills the supervisor first so it cannot relaunch. Wrapped by streamdeck/restart_bot.bat / stop_bot.bat.
  make_streamdeck_icons.py  Regenerates assets/streamdeck_icons/ (288x288 PNG button icons, hatmas_theme.css palette, labels baked in). Add buttons to its ICONS dict and re-run. See docs/STREAM_DECK.md for the full deck layout.
  readiness_ui.py           The readiness check as a real window. Standalone aiohttp app on 127.0.0.1:8073+ (works when the bot is down) that imports the probes from check_stream_ready.py and renders them with a fix button next to every non-green row (start/stop bot via bot_ctl.ps1, launch OBS / MixItUp / SMITE 2, restart cloudflared, re-auth tokens, open recordings). POST /api/fix is whitelist-only. Opens as an Edge --app window. Launcher: streamdeck/readiness_ui.bat.
  check_stream_ready.py     Pre-stream readiness check. Runs ~12 concurrent end-to-end probes (bot dashboard, both Twitch tokens, OBS WebSocket + Smite 2 source, MixItUp, tracker.gg, public webserver, hatmaster.tv, cloudflared service, disk space, asset library, Spotify token, SMITE 2 process). ~400ms full / ~150ms with --quick. Exit 0 = ready, 1 = at least one FAIL. Use streamdeck/check_stream.bat for one-press Stream Deck workflow.
streamdeck/                 ALL Stream Deck-facing launchers in one folder — see docs/STREAM_DECK.md for the button layout and wiring guide. Every relative-path .bat here starts with pushd "%~dp0.." so it operates from the repo root. Button icons live in assets/streamdeck_icons/ (regenerate via tools/make_streamdeck_icons.py). run_bot.bat stays at the repo root (Task Scheduler + bot_ctl.ps1 reference it there).
  readiness_ui.bat          Opens the readiness panel (tools/readiness_ui.py) as an app window — probes + one-click fix buttons. The CHECK READY button points here.
  check_stream.bat          Console variant of the readiness check (tools/check_stream_ready.py). Colored report with hints, pauses 8s on success / 30s on failure.
  restart_bot.bat           Restart (or start) the bot via tools/bot_ctl.ps1. Safe to press whether the bot is up, down, or wedged.
  stop_bot.bat              Stop supervisor + bot. Hard kill (safe: atomic writes + SQLite WAL); typing "quit" in the bot console stays the graceful path.
  go_live.bat               Apply LIVE NOW badge on last 8 YouTube thumbnails. Pair with go_offline.bat at end of stream.
  go_offline.bat            Revert LIVE badges. Idempotent and partial-failure tolerant.
  start_factorio.bat        Launch Factorio with RCON bound to 127.0.0.1:27015. Reads FACTORIO_RCON_PASSWORD from core/config_local.py (single source of truth). After launch: Multiplayer -> Host saved game (RCON is NOT active in single player).
  process_recordings.bat    End-of-stream wrapper around tools/process_recordings.py. Runs the orchestrator, tees stdout+stderr to data/process_recordings.log, brief pause at the end so the summary stays on screen.
  sort_unknowns.bat         Interactive walkthrough of recordings/unknown/ (tools/sort_unknowns.py) with portrait-reference capture — run after process_recordings when clips land unidentified.
  thumbnail_studio.bat      Launch the interactive Thumbnail Studio (tools/thumbnail_studio.py, aiohttp on 127.0.0.1:8071+) and open it in the browser. Console stays open while the server runs.
  build_thumbnail.bat       Older prompt-driven thumbnail builder around tools/build_thumbnail.py. Interactive prompt flow that adapts to the chosen preset (1v1 / 1v2 / 2matches / 2gods / single); blank inputs skip the corresponding --flag; flip toggles invert whatever the preset already does.
  resolve_import.bat        DaVinci Resolve import launcher (tools/resolve_import.py). Absolute paths, no pushd.
  resolve_tiktok.bat        Vertical TikTok build launcher (tools/resolve_tiktok.py). Absolute paths, no pushd.
  discord_test.bat          Wrapper around tools/discord_test.py. Args pass straight through (e.g. `discord_test.bat --list`, `discord_test.bat <channel_id> "hello"`); pauses at the end so you can read the OK/FAIL line.
...
**Price formula:** Fair-value formula in `plugins/economy/fair_value.py:calculate_fair_value(wins, losses, K, D, A)`. Asymmetric: above 50% winrate scales aggressively with games (so a 100-game 80% comfort pick is worth meaningfully more than a 2-game 100% sample); below 50% stays bounded. Settlement does NOT snap the price straight to fair value: `directional_settlement_price()` guarantees a win always settles up and a loss always settles down. When fair value agrees with the outcome direction the price converges toward it (capped at `SETTLE_MAX_MOVE_PCT` = ±30% per match); when it disagrees (e.g. a loss on a god priced below fair value), that match applies the KDA-shaped delta instead. `tools/replay_economy.py` is the exception by design: a replay/reconcile re-anchors prices to lifetime fair value (it has no per-match sequence to walk directionally). See `HATMAS_MARKET_AIRTIGHT_DESIGN.md` and the constants block at the top of `fair_value.py` for tuning.
...
**Stream Deck integration.** `streamdeck\go_live.bat` (apply) and `streamdeck\go_offline.bat` (revert) are wired for one-press triggering. Pair them with the same buttons that start/end your stream session.
...
**Stream Deck integration.** `streamdeck\check_stream.bat` wraps the python tool and pauses 8s on success / 30s on failure so you can read the hints. Drag onto a "System: Open" button. Press ~30s before going live; green verdict means clear, anything red shows you exactly what to fix.
