...
- **Public Website** — `hatmaster.tv`: a home page with the live Twitch embed + YouTube/TikTok/Bluesky tabs, the Hatmas Market at `/market` (god prices, portfolios, trading with a Twitch or YouTube login), and the community/god-request page. One account per person: a viewer is a `user_uuid` (`core/users.py`) that can hold both a Twitch and a YouTube identity, and linking them merges the portfolios.
...
<!-- sources:start (generated by tools/gen_readme_sources.py) -->
- **Hatmaster Alert Box** (gamble, TTS, voicelines, economy, spin, bingo alerts; configure at `http://localhost:8069/alerts/layout`): `http://localhost:8069/overlay/alerts?box=main` (1920x1080, full scene)
- **Now playing**: `http://localhost:8069/overlay/nowplaying` (520x140)
- **God overlay**: `http://localhost:8069/overlay/god` (420x120)
- **Death counter**: `http://localhost:8069/overlay/deaths` (240x90)
- **Economy ticker**: `http://localhost:8069/overlays/economy_ticker.html` (1920x48)
- **Live match panel**: `http://localhost:8069/overlays/economy_match_live.html` (420x160)
- **YouTube player**: `http://localhost:8069/overlay/youtube_player` (640x360)
- **Bingo status**: `http://localhost:8069/overlays/bingo.html` (400x260)
- **Bingo cards**: `http://localhost:8069/overlay/bingo_cards` (420x520)
- Legacy sources (folded into the alert box, still served for now): `/overlay/sound_alerts`, `/overlay/tts`, `/overlay/voicelines`, `/overlays/cocaster.html`, `/overlays/economy_dividend.html`, `/overlays/economy_match_end.html`, `/overlays/economy_leaderboard.html`, `/overlays/economy_tradefeed.html`, `/overlays/economy_portfolio.html`, `/overlay/spin`
- The full list with live status: `http://localhost:8069/sources`
<!-- sources:end -->
